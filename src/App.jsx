...
import { MarkdownRenderer, extractTitleFromMarkdown } from "./markdown";
...
const SITEMAP = {
  ...
  // -- 最佳实践 --
  prompts:{title:"怎么跟它说话（提示词）",nav:"提示词",group:"最佳实践",icon:"💬"},
  workflows:{title:"常见工作流程",nav:"工作流",group:"最佳实践",icon:"🔄"},
  // -- 进阶 --
  ...
  platforms:{title:"平台与团队集成",nav:"平台",group:"参考资料",icon:"🔗"},
  exercises:{title:"动手练习",nav:"练习",group:"参考资料",icon:"🛠️"},
};
...
const TOP_GROUPS=["快速开始","使用入口","核心概念","进阶功能","最佳实践","参考资料"];
...
const CLAUDE_GUIDE_GROUPS=[
  ...
  {key:"entry-points",title:"使用入口",matchers:[8,9,10,11,14,17,40,47]},
...
function normalizeContentPath(input){
  return String(input||"")
    .replace(/^\.\.\/content\/upstream\//,"")
    .replace(/^\/?content\/upstream\//,"")
    .replace(/^\.?\//,"");
}

function getArticleNumber(file){
  const match=normalizeContentPath(file).match(/\/(\d+)-/);
...
function resolveGuideGroupConfig(category,file){
  const number=getArticleNumber(file);
  const groups=category==="claude-code"?CLAUDE_GUIDE_GROUPS:CODEX_GUIDE_GROUPS;
  return groups.find((group)=>group.matchers.includes(number))||groups[groups.length-1];
}

const ARTICLE_CONTENT_MAP=Object.fromEntries(
  Object.entries(ARTICLE_MODULES).map(([path,content])=>[normalizeContentPath(path),content]),
);
const ASSET_URL_MAP=Object.fromEntries(
  Object.entries(ASSET_MODULES).map(([path,url])=>[normalizeContentPath(path),url]),
);
const ARTICLE_LIBRARY=UPSTREAM_SECTIONS.map((section)=>{
...
const ARTICLE_BY_FILE=Object.fromEntries(ARTICLE_LIBRARY.map((article)=>[article.file,article]));

function getGuideDefinitions(category){
  return category==="claude-code"?CLAUDE_GUIDE_GROUPS:CODEX_GUIDE_GROUPS;
}

function getGuideArticles(category){
  const groups=getGuideDefinitions(category);
  const articles=ARTICLE_LIBRARY.filter((article)=>article.category===category).sort((a,b)=>a.number-b.number);
  return groups.map((group)=>({
    ...group,
    articles:articles.filter((article)=>article.guideGroup===group.key),
...
function resolveAssetPath(currentFile,target){
  ...
  const normalizedCurrent=normalizeContentPath(currentFile);
  const baseParts=normalizedCurrent.split("/").slice(0,-1);
  const assetKey=normalizeContentPath(new URL(target,"https://local/"+baseParts.join("/")+"/").pathname);
  return ASSET_URL_MAP[assetKey]||"";
}

function resolveMarkdownLink(currentFile,target){
  if(!target)return {type:"external",href:"#"};
  if(target.startsWith("#"))return {type:"external",href:target};
  if(/^https?:\/\//.test(target))return {type:"external",href:target};
  const [pathOnly,hash=""]=target.split("#");
  const normalizedCurrent=normalizeContentPath(currentFile);
  const baseParts=normalizedCurrent.split("/").slice(0,-1);
  const resolvedPath=normalizeContentPath(new URL(pathOnly||".","https://local/"+baseParts.join("/")+"/").pathname);
  if(resolvedPath.endsWith(".md")&&ARTICLE_BY_FILE[resolvedPath]){
    return {type:"article",file:resolvedPath,hash};
  }
  if(pathOnly){
    const assetHref=ASSET_URL_MAP[resolvedPath];
    if(assetHref)return {type:"external",href:assetHref};
  }
...
function ArticleViewPage({article,guideKey,goTo,openArticle}){
  ...
  return <div>
    ...
    <Card style={{padding:"18px 20px",marginBottom:20,background:C.bg3}}>
      <div style={{display:"grid",gap:8}}>
        <div style={{fontSize:13,color:C.text2}}>来源 commit：<code style={{fontFamily:M,background:"#fff",padding:"2px 6px",borderRadius:6}}>{(upstreamManifest.upstreamCommit||"").slice(0,7)||"待同步"}</code></div>
        <div style={{fontSize:13,color:C.text2}}>原文件路径：<code style={{fontFamily:M,background:"#fff",padding:"2px 6px",borderRadius:6}}>{article.file}</code></div>
      </div>
    </Card>
...
export default function App(){
  ...
  const [articleState,setArticleState]=useState({file:null,guide:"claudeGuide"});
  const openArticle=useCallback((file,guide)=>{
  ...
  const article=articleState.file?ARTICLE_BY_FILE[articleState.file]:null;
  const PageComp=page==="articleView"
    ?()=>article?<ArticleViewPage article={article} guideKey={articleState.guide} goTo={goTo} openArticle={openArticle}/>:<div/>
    :page==="claudeGuide"
      ?()=> <GuideLandingPage category="claude-code" guideKey="claudeGuide" goTo={goTo} openArticle={openArticle}/>
      :page==="codexGuide"
  ...
  return <div style={{fontFamily:F,color:C.text,background:C.bg,height:"100vh",display:"flex",flexDirection:"column",WebkitFontSmoothing:"antialiased",fontSize:16}}>
    ...
    <nav style={{background:"rgba(251,251,253,.95)",backdropFilter:"saturate(180%) blur(20px)",WebkitBackdropFilter:"saturate(180%) blur(20px)",borderBottom:`1px solid ${C.borderLight}`,position:"sticky",top:0,zIndex:100}}>
      <div style={{maxWidth:layoutWidth,margin:"0 auto",padding:"0 32px",height:48,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div style={{display:"flex",gap:2,position:"relative",alignItems:"center"}}>
          {TOP_GROUPS.map(g=>{
            ...
            return <div key={g} style={{position:"relative"}} onMouseEnter={()=>setDropdown(g)} onMouseLeave={()=>setDropdown(null)}>
              <button style={{...navButtonStyle,fontWeight:isActive?600:400,color:isActive?C.text:C.text2,background:isActive?"rgba(0,0,0,.04)":"none"}}>{g}</button>
              {isOpen&&<div style={{position:"absolute",top:"calc(100% + 4px)",left:"50%",transform:"translateX(-50%)",zIndex:200}}>
                {renderTopNavDropdown(g)}
              </div>}
            </div>;
          })}
        </div>
    ...
    <div ref={contentRef} style={{flex:1,overflow:"auto"}} onScroll={e=>{setShowTop(e.target.scrollTop>400)}}>
      <div style={{maxWidth:layoutWidth,margin:"0 auto",padding:page==="home"?"0":"40px 32px 80px"}}>
        {showLinearNav&&<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24,paddingBottom:14,borderBottom:`1px solid ${C.borderLight}`}}>
        ...
        <PageComp goTo={goTo}/>
        {showLinearNav&&next&&<div style={{marginTop:48,padding:24,background:C.bg3,borderRadius:16,textAlign:"center"}}>
